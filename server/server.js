...
app.use(cors({
  origin: "http://localhost:3000",  // or whatever your React app’s URL is
  credentials: true
}));
  ...
  return jwt.sign(
    {
      ...
      userType: user.UserType,
       username: user.Id,
      email: user.Id // assuming Id is email
    },
    JWT_SECRET,
...
const IsAdmin = (req, res, next) => {
  // First verify the token is valid (should come after verifyToken middleware)
  if (!req.user) {
    return res.status(401).json({
      error: {
        code: 401,
...
app.post('/login_auth', (req, res) => {
  ...
  console.log("this is usernmae and passsword to login: "+ username+ ", "+ password);
...
app.get('/viewNews', verifyToken, IsAdmin, (req, res) => {
...
app.get('/viewalumni',verifyToken,IsAdmin,(req, res) => {
...
app.get('/delete_faculty/:id',verifyToken, IsAdmin,(req, res) => {
...
app.get('/uploadNews',verifyToken, IsAdmin, (req, res) => {
...
app.get('/faculty_experiences', verifyToken, (req, res) => {
...
app.get('/update_faculty_experience', verifyToken, (req, res) => {
  const username = req.user.username;
  var sql2 = 'SELECT * FROM railway.profession_career WHERE email="'+username+'";';
  connection.query(sql2, function (err, data) {
    if (err){
...
app.get('/faculty_awards', verifyToken, (req, res) => {
  const username = req.user.username;
...
app.get('/faculty_qualifications', verifyToken, (req, res) => {
  const username = req.user.username;
...
app.post('/add_faculty_publication', verifyToken, (req, res) => {
...
app.get('/update_faculty_publications', verifyToken, (req, res) => {
  const username = req.user.username;
...
app.get('/faculty_publications',verifyToken,  (req, res) => {
  const username = req.user.username;
 // console.log("username",username);
  var sql2 = 'SELECT * FROM railway.publication WHERE email="'+username+'";';
  connection.query(sql2, function (err, data) {
...
app.get('/faculty_dashboard', verifyToken, (req, res) => {
...
app.get('/delete_news/:id/:file?',verifyToken, IsAdmin, (req, res) => {
...
app.post('/reg_sub', verifyToken, upload.single('photo') , (req, res, next)=>{
  try{  
...
app.post(
  '/update_faculty_details', verifyToken,
  upload.fields([
...
app.post('/update_faculty_experience',verifyToken, (req, res, next) => {
...
app.post('/update_faculty_award',verifyToken, (req, res, next) => {
  console.log(req.body);
...
app.post('/update_faculty_qualification',verifyToken, (req, res, next) => {
  console.log(req.body);
...
app.post('/update_faculty_publication',verifyToken, (req, res, next) => {
  console.log(req.body);
...
app.get('/addfaculty', verifyToken, (req, res) => {
...
app.get("/checkAuth", verifyToken, (req, res) => {
  if (req.user.userType === "admin") {
    ...
    const sql = 'SELECT * FROM railway.faculty WHERE Id = ?';
    connection.query(sql, [req.user.username], (err, data) => {
      if (err) {
        console.error('Database error:', err);
        return res.status(500).json({ message: 'Database error' });
      }
